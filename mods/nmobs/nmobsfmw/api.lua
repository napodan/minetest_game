...
function mobs_check_state(self, dtime)
...
function mobs_on_step(self, dtime)

	mobs_check_state(self, dtime)
	...
	if self.type == "warpet" and self.attacks_monsters and self.state ~= "attack" and self.following == player then
		...
		for _, oir in pairs(inradius) do
			local obj = oir:get_luaentity()
			if obj then
				if obj.type == "monster" and self.object:getpos().y <= obj.object:getpos().y then
					self.following = nil
			...
			if dist > self.view_range then
				...
				if dist > 2 then
					if not self.v_start then
						...
						if self.jump and self.get_velocity(self) <= (self.walk_velocity /2) and self.object:getvelocity().y == 0 then
